...
- [x] Published `@antubattle/harness` publicly and verified the result.
...
- `npm version minor` created the local `0.2.0` commit and tag before the final release-content commit existed, so the tag had to be moved to the final release commit before pushing and publishing.
...
- `@antubattle/harness@0.2.0` is now published, `npm view` confirms the released metadata, and the release branch plus `v0.2.0` tag are pushed to GitHub.
- The final release workflow required moving the `v0.2.0` tag from the initial `npm version minor` commit onto the final release-content commit so the Git tag and published tarball matched.
- Local validation succeeded with `npm test`, `npm run smoke:pack`, and `npm pack --dry-run`, and the final independent validation pass reported no material issues.
