...
10. The generated guidance for `features/generated/` must require runtime logs, validation output, screenshots, traces, and similar artifacts to be stored under day-based subdirectories such as `features/generated/YYYY-MM-DD/`, while keeping `features/generated/README.md` at the directory root.
11. The generated `harness.config.json` must record:
   - Harness version
   - scaffold version
   - project name
12. If any generated path already exists, `init` must fail before writing and must identify the conflicting files.
13. The command must emit a concise, deterministic summary of the files it created.
...
- The generated scaffold includes `REVIEW.md` and `PRODUCT_SPECS.md`, and the generated `AGENTS.md` references both.
- The generated workflow docs explicitly require a separate validation-subagent closure loop, day-based folders under `features/generated/`, validation at the edge, and single-purpose Conventional Commits.
